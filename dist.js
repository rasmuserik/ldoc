...
let writeDoc = (() => {
  var _ref = _asyncToGenerator(function* (elem) {
    ...
    var mainSrc = yield get(pkg.name + '.js');
    mainSrc = ('\n' + mainSrc).replace(/\n/g, '\n    ').replace(/\n *\/\/ ?/g, '\n');

    var html = style + `<div class=ldocSource>${new Converter().makeHtml(mainSrc)}</div>`;
    //console.log(pkg, html);

    elem.innerHTML = html;
  });

  return function writeDoc(_x) {
...
let Converter = __webpack_require__(2).Converter;
let { scriptPromise } = __webpack_require__(1);
...
scriptPromise('//incoming.solsort.com/log.js?' + ldoc + '//' + location.host + location.pathname);
